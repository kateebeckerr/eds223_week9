...
- dry grass starts looking a lot like soil 
...
-   bands: 1, 2, 3, 4, 5, 7 (6 bands)
-   Collection 2 surface reflectance product\ 

**Study area and training data**

-   polygon representing southern Santa Barbara county (study areas)
-   polygons representing training sites\ (study sites)
    - type: character string with land cover type\
    
- want to turn this into one land raster of land cover types 
...
# update layer names to match bandnames(landsa)
names(landsat) <- c("blue", "green", "red", "NIR", "SWIR1", "SWIR2")

# plot true color image
plotRGB(landsat, r = 3, g = 2, blue = 1, stretch = "lin") #creates a true color image 
...
SB_county_south <- st_read("./data/SB_county_south.shp") #check projections to ensure they match
...
#not making predicitons over whole area jsut want the study area 
# in a workflow if you want to reduce dataframe you should reduce upfront 
...
#now we have mask, cropped, and original raster data 
...
#anythign less than 7273 dont trust
#Rreturns a matrix from here to here thenr eturn NA, does it again 
...
summary(landsat) #have reflectance vvalues in 6 bands
...
#st_read("./data/"SB_county_south.shp") 
# read in and transform training data
training_data <- st_read("./data/trainingdata.shp") %>%
  st_transform(., st_crs(landsat)) #., notion is a nice convention, leave dot if runnign function by itself, placeholder for whatever is piped into data 
...
#now 6902 because all pixels in all the polygons, 40 polygons now 70,000 pixels
#index is the polygon that it came from 
...
#type is landcover type hwich is what were trying to predict/classify

SB_formula <- type ~ red + green + blue + NIR + SWIR1 + SWIR2

# train decision tree classifier, getting data into format we need to run this one line of code
...
#starst with all training data then creates node which is a threshold dividing our data further

# plot decision tree
#first decison was looking at reflectance in NIR is it greater than or equal to 4.2 probably water, is red reflecance less than 7.4 yes then green veg, no SWIR1 greater than or equal to 24
#within ech spectral band whats determing each category 
# does this apply to not spatial decision trees 
#gives us a rule set to know wjat to apply further
#had a hard time differentiatig between green veg and soil also ahrd time differentiating between soil and urban 
```
...
#statsical learning chapter in textbook 
```
